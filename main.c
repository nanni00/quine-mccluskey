...
{
	/*
	 * n_dontcares = numero dontcares, "don't care"
	 * n_only_one = numero mintermini senza dontcares
	 ...
	 */
	 ...
	 /*
		 ...
		 1 --> n_mintermini ON_SET escluse le dontcares
		 2 --> n_dontcares
	...
	while (only_one[general_n[1] - 1] < pow(2, general_n[0] - 1)) {
		if (n_dontcares != 0 && dontcares[general_n[2] - 1] >= pow(2, general_n[0] - 1))
			break;
		--general_n[0];
	}
	...
	/* Se i mintermini sono più della metà del numero di configurazioni possibili (dontcares escluse) */
	...
	Implic *v = readMinterms(index, mintermini, general_n);
    writeImplicant(general_n[3], general_n, v, f);
	if (ShowWork) writeImplicant(general_n[3], general_n, v, g);


	printf("\n\n\nSTEP 1:\n\n");
	Implic *primi = stepOne(general_n, dontcares, v, ShowWork);

	printf("\nPrime implicants found:\t%zu\n", general_n[4]);
    writeImplicant(general_n[4], general_n, primi, f);
	if (ShowWork) writeImplicant(general_n[4], general_n, primi, g);
	free(index);
	...
	if (1) {
		printf("\n\n\nSTEP 2:\n\n");
		Implic *essenziali = stepTwo(general_n, mintermini, primi, ShowWork);

		printf("\nEssential prime implicants found:\t%zu\n", general_n[5]);
        writeImplicant(general_n[5], general_n, essenziali, f);
		if (ShowWork) writeImplicant(general_n[5], general_n, essenziali, g);
		printf("\n");
        ...
        deleteImplicantsVector(general_n[5], essenziali);
	}
    deleteImplicantsVector(general_n[4], primi);
